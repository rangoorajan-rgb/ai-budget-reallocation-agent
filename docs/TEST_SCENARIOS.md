...
## Final Integration Scenarios (Stage 36, `tests/test_integration.py`)

| # | Scenario | Expected outcome |
|---|---|---|
| A | Approved flow: real sample CSV upload through real deterministic pipeline, approve, audit, export | `total_current_budget`/`total_recommended_budget = 11700.00`, `is_conserved = True`; campaign order `G001, M001, G002, G003`; G001/M001/G003 `MAINTAIN` (`0.00` allocated, `Not ranked`, `NEAR_TARGET, RECENT_TREND_STABLE`); G002 `INCREASE` (`0.00` allocated, rank `1`, `ABOVE_TARGET_STRONG, RECENT_TREND_IMPROVING`); the captured real CSV (never mocked) matches every field of the stored `CampaignReallocationAudit` exactly. |
| B | Rejected flow: same upload, reject | Decision finalizes `REJECTED`; locked recommendations unchanged (`model_dump()` equality); one audit JSON persists recording `REJECTED`; every exported CSV row records `REJECTED`; no reconsider/overwrite control renders. |
| C | Fake `GENERATED` portfolio and campaign explanations | Text renders, labeled AI-generated/supplementary; exactly one fake call per click, zero on a bare rerun; locked result's `model_dump()` unchanged; fake explanation text absent from both the persisted audit JSON and the exported CSV; approval/audit/export remain fully functional afterward. |
| D | Real, network-free `UNAVAILABLE` Gemini path, one explanation click | Does not block approval, audit persistence, or CSV export availability. |
| E | Unconserved result injected via the established pipeline-wrapping test-fixture pattern (no production code touched) | Approval blocked with the exact existing conservation error; rejection remains allowed, finalizes, persists, and exports; every CSV row records `REJECTED` and `is_conserved=False`; nothing repaired, rebalanced, recalculated, or rerun. |
| F | Audit-persistence failure on first attempt, successful retry | Decision remains finalized after the failure with the exact sanitized error (no raw path/traceback); export control absent until the retry succeeds; exactly one audit JSON exists afterward; locked result and approval unchanged across failure and retry. |
| G | Invalid `ReviewSetup` submitted through the UI | Genuine validation issues render; pipeline does not run; every downstream state key (explanation, approval, audit path/error/object) stays empty; no export control; no audit JSON written. |
| H | CSV with at least one valid row and one malformed row | Validation issue renders; the whole portfolio is blocked; no partial locked result; no downstream state; no audit JSON written. |
| I | New valid, then new invalid, submission after a completed audited/exported cycle | Both clear every downstream state key (`locked_review_result` is replaced, not merely cleared, for the valid case); the export control disappears in both cases. |
| J | Bare rerun (no button clicked) after a completed cycle | Locked result, explanation result, approval, audit path, stored audit object, audit-file count, and captured CSV are all byte-identical; zero additional pipeline, Gemini, or audit-persistence calls. |
| Cross-cutting | A synthetic fake API key, set via the environment, across a full approve-and-explain cycle | Never appears in any rendered element, session-state value, the persisted audit JSON, or the exported CSV; fake explanation text is likewise absent from the audit JSON and CSV; the real repository `audit_records/` directory and overall file listing are unchanged before and after; no `exports/` directory is ever created. |
